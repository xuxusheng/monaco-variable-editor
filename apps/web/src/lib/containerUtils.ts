...
export function collectDescendants(containerId: string, nodes: WorkflowNode[]): string[] {
  const result: string[] = [];
...
export function filterVisibleNodes(nodes: WorkflowNode[]): WorkflowNode[] {
  const collapsedIds = new Set(nodes.filter((n) => n.ui?.collapsed).map((n) => n.id));
...
/** 计算容器的直接子节点数量 */
export function getChildCount(containerId: string, nodes: WorkflowNode[]): number {
  return nodes.filter((n) => n.containerId === containerId).length;
}

/** 获取容器的所有直接子节点 */
export function getChildren(containerId: string, nodes: WorkflowNode[]): WorkflowNode[] {
  return nodes
 ...
 */
export function canExpandContainer(containerId: string, nodes: WorkflowNode[]): boolean {
